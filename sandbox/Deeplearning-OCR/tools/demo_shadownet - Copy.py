...
def recognize(image_path, weights_path, is_vis=True):
    ...
    cost = tf.nn.ctc_loss(labels=input_labels, inputs=net_out, sequence_length=25*np.ones(1), merge_repeated=True)
    decodes, _ = tf.nn.ctc_beam_search_decoder(inputs=net_out, sequence_length=25*np.ones(1), merge_repeated=True)
    ...
    with sess.as_default():
		...
		loss = sess.run(cost, feed_dict={inputdata: image,labels: sparse})
